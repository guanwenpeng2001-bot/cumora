...
const VERSIONED_MIGRATIONS: readonly VersionedMigration[] = [
  ...
  },
  {
    ...
    sourceChecksum: usageRollupV2Checksum(),
    transactional: true,
    up: async (client) => { await client.query(USAGE_ROLLUP_V2_SQL) },
  },
  {
    ...SCHEMA_MIGRATIONS[13],
    sourceChecksum: runtimeCallIdIndexChecksum(),
    transactional: true,
    up: async (client) => { await client.query(RUNTIME_CALL_ID_INDEX_SQL) },
  },
  {
    ...SCHEMA_MIGRATIONS[14],
    sourceChecksum: engineDefaultsChecksum(),
    transactional: true,
    up: async (client) => { await client.query(ENGINE_DEFAULTS_SQL) },
  },
  {
  ...
  },
  {
    ...
    sourceChecksum: turnSafetyChecksum(),
    transactional: true,
    up: async (client) => { await client.query(TURN_SAFETY_SQL) },
  },
  {
    ...SCHEMA_MIGRATIONS[18],
    sourceChecksum: modelHubChecksum(),
    transactional: true,
