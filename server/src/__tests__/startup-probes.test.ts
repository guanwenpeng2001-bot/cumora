...
const { load, loadAll } = createRequire(import.meta.url)('js-yaml') as {
  load: (yaml: string) => unknown
  loadAll: (yaml: string) => unknown[]
}
...
function durationMs(value: string | undefined, label: string): number {
  assert.ok(value, `${label} is required`)
  const match = /^(\d+)s$/.exec(value)
  assert.ok(match, `${label} must be a seconds duration, got ${value}`)
...
test('the deploy patch reapplies the same probe contract and rollout deadline', async (t) => {
  ...
  try {
    ...
    // Windows and some CI images have no jq(1). The regex contract above
    // still pins the probe patch; skip the live jq evaluation when the
    // binary is missing rather than failing the suite.
    if (code === 'ENOENT' || (error instanceof Error && /spawn jq ENOENT/i.test(error.message))) {
      t.skip('jq is unavailable; regex contract above still pins the probe patch')
      return
    }
    throw error
  }
...
test('compose dependency healthcheck grace covers the schema boot retry budget', async () => {
  ...
  const boundedConnectionMs = attempts * connectionTimeoutMs
  assert.ok(
    startPeriodMs > retryDelayMs + boundedConnectionMs,
    `compose start_period ${startPeriodMs}ms must exceed ${retryDelayMs + boundedConnectionMs}ms`,
  )
})
