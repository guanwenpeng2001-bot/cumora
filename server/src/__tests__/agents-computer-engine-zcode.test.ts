...
import { ENGINE_IDS, SANDBOXED_ENGINE_IDS, getAdapter, runnableEngineIds, type EngineHopReport } from '../agents/computer/engine.js'
...
test('zcode joins the engine registry alongside kimi and stays unsandboxed', () => {
  assert.ok(ENGINE_IDS.includes('zcode'))
  assert.ok(ENGINE_IDS.includes('kimi'), 'porting zcode must not drop kimi')
  assert.ok(!SANDBOXED_ENGINE_IDS.includes('zcode'))
  assert.deepEqual(runnableEngineIds(['zcode', 'kimi'], {}), [])
  assert.deepEqual(
    runnableEngineIds(['zcode', 'kimi'], { CUMORA_BYOA_ALLOW_UNSANDBOXED: '1' }),
    ['zcode', 'kimi'],
  )
})
