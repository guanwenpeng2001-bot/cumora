...
test('provider profiles append version 16 without changing the fork migration prefix', async () => {
  const { AGENT_PROVIDER_PROFILE_SQL, agentProviderProfileChecksum } = await import('../db/migrations/0016-agent-provider-profile.js')
  ...
  assert.doesNotMatch(AGENT_PROVIDER_PROFILE_SQL, /model_config|api_key|auth_token/i)
})
