...
/** lstat also rejects links on Windows, where O_NOFOLLOW is unavailable.
 * O_NOFOLLOW remains the atomic link protection on supported POSIX systems. */
function checkLocalFile(path: string): void {
  try {
    const stat = lstatSync(path)
    if (!stat.isFile() || stat.isSymbolicLink()) throw new Error()
  } catch (err) {
    if ((err as NodeJS.ErrnoException).code === 'ENOENT') return
    throw new Error('providers.json must be a regular local file')
  }
}
...
export function readProviderProfiles(path: string): ProviderProfile[] {
  ...
  try {
    ...
    if (!stat.isFile() || stat.size > MAX_PROFILE_BYTES) throw new Error('providers.json must be a regular file under 1 MiB')
...
/** Local operator API only: never expose this through the server/router.
 * Validate before touching disk, then atomically replace a same-directory 0600
 * file. O_EXCL/O_NOFOLLOW prevents following a pre-existing staging-file link;
 * rename replaces the destination entry rather than opening its target. */
export function writeProviderProfiles(path: string, raw: unknown): void {
  const profiles = parseProviderProfiles(raw)
  const data = `${JSON.stringify({ version: 1, profiles }, null, 2)}
`
  if (Buffer.byteLength(data) > MAX_PROFILE_BYTES) throw new Error('providers.json must be under 1 MiB')
  checkLocalFile(path)
  ...
  try {
    ...
    // Filesystem errors can include caller-controlled paths; keep them local
    // and never echo raw configuration or credentials in diagnostics.
    throw new Error('cannot write providers.json securely')
  } finally {
...
export function redactProviderSecret(text: string, profile?: ProviderProfile): string {
  ...
  // Engine JSON logs can escape quotes and backslashes in a credential.
  const variants = [secret, JSON.stringify(secret).slice(1, -1)]
  try { variants.push(encodeURIComponent(secret)) } catch { /* malformed Unicode still gets literal/JSON redaction */ }
  return variants.reduce((redacted, value) => redacted.split(value).join('[redacted]'), text)
}
