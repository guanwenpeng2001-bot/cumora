...
test('email handler keeps the synthesized id across a tempfail retry with new Received headers', async (t) => {
  t.mock.timers.enable({ apis: ['Date'], now: new Date('2026-09-08T09:20:00Z') })
  const posted: Array<{ messageId: string }> = []
  const rejected: string[] = []
  t.mock.method(globalThis, 'fetch', async (_url: unknown, init: RequestInit) => {
    posted.push(JSON.parse(String(init.body)))
    return new Response('', { status: posted.length === 1 ? 502 : 200 })
  })
  const message = (received: string) => ({
    to: sameMessage.envelopeTo,
    from: sameMessage.envelopeFrom,
    raw: new Response([
      `Received: ${received}`,
      `From: ${sameMessage.envelopeFrom}`,
      `To: ${sameMessage.envelopeTo}`,
      `Date: ${sameMessage.date}`,
      `Subject: ${sameMessage.subject}`,
      '', sameMessage.body,
    ].join('\r\n')).body!,
    setReject: (reason: string) => { rejected.push(reason) },
  }) as unknown as Parameters<typeof worker.email>[0]

  await assert.rejects(() => worker.email(message('first hop'), fakeEnv, fakeCtx), /Upstream temporary failure/)
  t.mock.timers.tick(2 * SYNTH_ID_BUCKET_MS)
  await worker.email(message('retry hop'), fakeEnv, fakeCtx)

  assert.deepEqual(rejected, [])
  assert.equal(posted.length, 2)
  assert.match(posted[0].messageId, /^synth-[0-9a-f]{64}@cumora-email-gate$/)
  assert.equal(posted[1].messageId, posted[0].messageId)
})

test('email handler preserves an existing Message-Id', async (t) => {
  const posted: Array<{ messageId: string }> = []
  t.mock.method(globalThis, 'fetch', async (_url: unknown, init: RequestInit) => {
    posted.push(JSON.parse(String(init.body)))
  ...
  await worker.email(message, fakeEnv, fakeCtx)
  assert.deepEqual(rejected, [])
  assert.deepEqual(posted.map((p) => p.messageId), ['msg-1@example.com'])
})

test('undated synthesized ids are stable within a bucket and change at its boundary', async (t) => {
  t.mock.timers.enable({ apis: ['Date'], now: 10 * SYNTH_ID_BUCKET_MS })
  const parts = { ...sameMessage, date: undefined }
  const first = await synthesizeMessageId(parts)
  t.mock.timers.tick(SYNTH_ID_BUCKET_MS - 1)
  assert.equal(await synthesizeMessageId(parts), first)
  t.mock.timers.tick(1)
  assert.notEqual(await synthesizeMessageId(parts), first)
})
